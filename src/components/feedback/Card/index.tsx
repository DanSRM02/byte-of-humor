...
const Card = ({
  ...
  if (variant === "expandable") {
    return (
      <article
        ...
        tabIndex={0}
        aria-label={title + (badge ? `, ${badge}` : "") + (isExpanded ? ", expanded" : ", collapsed")}
        role="button"
        ...
        <header className={classes["card__header"]}>
          <div className={classes["card__header-content"]}>
            <div className={classes["card__icon-wrapper"]}>
              {icon && <div className={classes["card__icon"]} aria-hidden="true">{icon}</div>}
            </div>
              ...
              <div className={classes["card__features"]}>
                {features.map((feature, index) => (
                  <div key={index} className={classes["card__feature"]}>
                    <div className={classes["card__feature-dot"]} aria-hidden="true"></div>
                    <span className={classes["card__feature-text"]}>{feature}</span>
                  </div>
                ))}
              </div>
              {onExplore && (
  ...
  if (variant === "joke") {
    return (
      <article className={`${classes["card"]} ${classes["card--joke"]}`} tabIndex={0} aria-label={title}>
        {title && <h6 className={classes["card__category-title"]}>{title}</h6>}
        {jokeType === "twopart" ? (
          <>
            <p
  ...
  return (
    <article className={classes["card"]} tabIndex={0} aria-label={title}>
      ...
      <div className={classes["card__main-content"]}>
        {icon && (
          <div className={classes["card__icon-wrapper"]}>
            <div className={classes["card__icon"]} aria-hidden="true">{icon}</div>
          </div>
        )}
